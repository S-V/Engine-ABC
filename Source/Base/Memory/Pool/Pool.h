...
=============================================================================
	File:	Pool.h
	Desc:	An optimised way of performing dynamic memory allocation, using a
	...
	Note:	This was 'CPool' swiped from flipcode.
=============================================================================
*/
...
mxMemoryPool< T >::mxMemoryPool( const WORD nPoolSize )
	...
	, m_nTOS( 0 )
{
	m_pPool = reinterpret_cast< BYTE* >( new BYTE [sizeof(T) * m_nPoolSize] );
...
template< class T >
void * operator new( size_t size, ::abc::mxMemoryPool<T> & rMemPool )
{
	return rMemPool.New( size );
}

template< class T >
void operator delete( void* p, ::abc::mxMemoryPool<T> & rMemPool )
{
