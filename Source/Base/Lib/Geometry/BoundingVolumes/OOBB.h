...
public:
	...
	void			FromBoxRotation( const OOBB &box, const mxRotation &rotation );
...
FORCEINLINE void OOBB::Clear( void ) {
	center.SetZero();
	extents[0] = extents[1] = extents[2] = -mxMath::INFINITY;
	axis.SetIdentity();
}
...
FORCEINLINE bool OOBB::ContainsPoint( const Vec3D &p ) const {
	Vec3D lp = p - center;
	if ( mxMath::Fabs( lp * axis[0] ) > extents[0] ||
			mxMath::Fabs( lp * axis[1] ) > extents[1] ||
				mxMath::Fabs( lp * axis[2] ) > extents[2] ) {
		return false;
	}
	return true;
}
...
FORCEINLINE void OOBB::AxisProjection( const Vec3D &dir, FLOAT &min, FLOAT &max ) const {
	FLOAT d1 = dir * center;
	FLOAT d2 = mxMath::Fabs( extents[0] * ( dir * axis[0] ) ) +
...
FORCEINLINE void OOBB::AxisProjection( const Matrix3 &ax, AABB &bounds ) const {
	for ( int i = 0; i < 3; i++ ) {
		...
		FLOAT d2 = mxMath::Fabs( extents[0] * ( ax[i] * axis[0] ) ) +
					mxMath::Fabs( extents[1] * ( ax[i] * axis[1] ) ) +
					mxMath::Fabs( extents[2] * ( ax[i] * axis[2] ) );
		bounds[0][i] = d1 - d2;
		bounds[1][i] = d1 + d2;
	}
}
