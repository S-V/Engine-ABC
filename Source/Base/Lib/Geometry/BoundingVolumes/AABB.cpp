...
const AABB AABB::BOUNDS_ZERO		( Vec3D::ZERO, Vec3D::ZERO );
const AABB AABB::INFINITE_EXTENT	( Vec3D::NEGATIVE_INFINITY, Vec3D::POSITIVE_INFINITY );
...
FLOAT AABB::GetRadius( void ) const {
	...
	total = 0.0f;
	for ( i = 0; i < 3; i++ ) {
		b0 = (FLOAT)mxMath::Fabs( mPoints[0][i] );
		b1 = (FLOAT)mxMath::Fabs( mPoints[1][i] );
		if ( b0 > b1 ) {
			total += b0 * b0;
		} else {
			total += b1 * b1;
...
FLOAT AABB::GetRadius( const Vec3D &center ) const {
	...
	total = 0.0f;
	for ( i = 0; i < 3; i++ ) {
		b0 = (FLOAT)mxMath::Fabs( center[i] - mPoints[0][i] );
		b1 = (FLOAT)mxMath::Fabs( mPoints[1][i] - center[i] );
		if ( b0 > b1 ) {
			total += b0 * b0;
		} else {
			total += b1 * b1;
...
================
*/
FLOAT AABB::PlaneDistance( const mxPlane &plane ) const
{
	Vec3D center;
	FLOAT d1, d2;
	...
	d2 = mxMath::Fabs( ( mPoints[1][0] - center[0] ) * plane.Normal()[0] ) +
			mxMath::Fabs( ( mPoints[1][1] - center[1] ) * plane.Normal()[1] ) +
				mxMath::Fabs( ( mPoints[1][2] - center[2] ) * plane.Normal()[2] );
...
================
*/
EPlaneSide AABB::PlaneSide( const mxPlane &plane, const FLOAT epsilon ) const
{
	Vec3D center;
	FLOAT d1, d2;
...
bool AABB::LineIntersection( const Vec3D &start, const Vec3D &end ) const {
    ...
    ld[0] = mxMath::Fabs( lineDir[0] );
	if ( mxMath::Fabs( dir[0] ) > extents[0] + ld[0] ) {
        return false;
	}

    ld[1] = mxMath::Fabs( lineDir[1] );
	if ( mxMath::Fabs( dir[1] ) > extents[1] + ld[1] ) {
        return false;
	}

    ld[2] = mxMath::Fabs( lineDir[2] );
	if ( mxMath::Fabs( dir[2] ) > extents[2] + ld[2] ) {
        return false;
	}

    Vec3D cross = lineDir.Cross( dir );

	if ( mxMath::Fabs( cross[0] ) > extents[1] * ld[2] + extents[2] * ld[1] ) {
        return false;
	}

	if ( mxMath::Fabs( cross[1] ) > extents[0] * ld[2] + extents[2] * ld[0] ) {
        return false;
	}

	if ( mxMath::Fabs( cross[2] ) > extents[0] * ld[1] + extents[1] * ld[0] ) {
        return false;
	}
...
bool AABB::RayIntersection( const Vec3D &start, const Vec3D &dir, FLOAT &scale ) const {
	...
	for ( i = 0; i < 3; i++ ) {
		...
		if ( ax0 < 0 || mxMath::Fabs( f ) > mxMath::Fabs( scale * dir[i] ) ) {
			scale = - ( f / dir[i] );
			ax0 = i;
		}
	}
...
void AABB::FromTransformedBounds( const AABB &bounds, const Vec3D &origin, const Matrix3 &axis ) {
	...
	for ( i = 0; i < 3; i++ ) {
		rotatedExtents[i] = mxMath::Fabs( extents[0] * axis[0][i] ) +
							mxMath::Fabs( extents[1] * axis[1][i] ) +
							mxMath::Fabs( extents[2] * axis[2][i] );
	}
...
*/
#if 0
void AABB::FromPoints( const Vec3D *points, const mxUInt numPoints ) {
...
================
*/
static AABB BoundsForPointRotation( const Vec3D &start, const mxRotation &rotation )
{
	int i;
	FLOAT radiusSqr;
	Vec3D v1, v2;
		...
		if ( ( v1[i] > 0.0f && v2[i] < 0.0f ) || ( v1[i] < 0.0f && v2[i] > 0.0f ) ) {
			if ( ( 0.5f * (start[i] + end[i]) - origin[i] ) > 0.0f ) {
				...
				if( axis[i] * axis[i] < 1.0f ) {
					bounds[1][i] += mxMath::Sqrt( radiusSqr * ( 1.0f - axis[i] * axis[i] ) );
				}
			}
			else {
				bounds[0][i] = origin[i];
				if( axis[i] * axis[i] < 1.0f ) {
					bounds[0][i] -= mxMath::Sqrt( radiusSqr * ( 1.0f - axis[i] * axis[i] ) );
				}
...
============
*/
void AABB::FromPointRotation( const Vec3D &point, const mxRotation &rotation ) {
	FLOAT radius;

	if ( mxMath::Fabs( rotation.GetAngle() ) < 180.0f ) {
...
============
*/
void AABB::FromBoundsRotation( const AABB &bounds, const Vec3D &origin, const Matrix3 &axis, const mxRotation &rotation ) {
	mxUInt i;
	FLOAT radius;
	Vec3D point;
	AABB rBounds;
...
============
*/
const mxUInt* AABB::GetEdges() const
{
	static mxUInt indices[ 12*2 ] = 
	{
		0, 1,
		1, 2,
		2, 3,
...
============
*/
const Vec3D* AABB::GetVertexNormals() const
{
	MX_TODO("scale normals properly, using minPoint and maxPoint")
	static const Vec3D vertexNormals[ 8 ] = 
	{
		Vec3D( -mxMath::SQRT_1OVER3,	-mxMath::SQRT_1OVER3,	-mxMath::SQRT_1OVER3	),
		Vec3D( mxMath::SQRT_1OVER3,		-mxMath::SQRT_1OVER3,	-mxMath::SQRT_1OVER3	),
		Vec3D( mxMath::SQRT_1OVER3,		mxMath::SQRT_1OVER3,	-mxMath::SQRT_1OVER3	),
		Vec3D( -mxMath::SQRT_1OVER3,	mxMath::SQRT_1OVER3,	-mxMath::SQRT_1OVER3	),
...
============
*/
void AABB::GetPlanes( mxPlane planes[NUM_SIDES] ) const
{
	Vec3D  corners[ NUM_CORNERS ];
	GetCorners( corners );

	planes[ AABB_SIDE_ABOVE ]	.FromPoints( corners[3], corners[2], corners[6] );
	planes[ AABB_SIDE_BENEATH ]	.FromPoints( corners[1], corners[0], corners[4] );
	
	planes[ AABB_SIDE_IN_FRONT ].FromPoints( corners[5], corners[4], corners[7] );
	planes[ AABB_SIDE_BEHIND ]	.FromPoints( corners[0], corners[1], corners[2] );
	
	planes[ AABB_SIDE_LEFT ]	.FromPoints( corners[0], corners[3], corners[7] );
	planes[ AABB_SIDE_RIGHT ]	.FromPoints( corners[1], corners[5], corners[6] );


	for ( mxUInt iSide = 0; iSide < NUM_SIDES; iSide++ )
	{
