...
const OOBB OOBB::box_zero( Vec3D::ZERO, Vec3D::ZERO, Matrix3::mat3_identity );
...
================
*/
FLOAT OOBB::PlaneDistance( const mxPlane &plane ) const {
	FLOAT d1, d2;

	d1 = plane.Distance( center );
	d2 = mxMath::Fabs( extents[0] * plane.Normal()[0] ) +
			mxMath::Fabs( extents[1] * plane.Normal()[1] ) +
				mxMath::Fabs( extents[2] * plane.Normal()[2] );
...
================
*/
EPlaneSide OOBB::PlaneSide( const mxPlane &plane, const FLOAT epsilon ) const {
	FLOAT d1, d2;
...
bool OOBB::IntersectsBox( const OOBB &a ) const {
    ...
    ac[0][0] = mxMath::Fabs( c[0][0] );
    ac[0][1] = mxMath::Fabs( c[0][1] );
    ac[0][2] = mxMath::Fabs( c[0][2] );

    d = mxMath::Fabs( axisdir[0] );
	e0 = extents[0];
    ...
    ac[1][0] = mxMath::Fabs( c[1][0] );
    ac[1][1] = mxMath::Fabs( c[1][1] );
    ac[1][2] = mxMath::Fabs( c[1][2] );

    d = mxMath::Fabs( axisdir[1] );
	e0 = extents[1];
    ...
    ac[2][0] = mxMath::Fabs( c[2][0] );
    ac[2][1] = mxMath::Fabs( c[2][1] );
    ac[2][2] = mxMath::Fabs( c[2][2] );

    d = mxMath::Fabs( axisdir[2] );
	e0 = extents[2];
    ...
    // axis C0 + t * A0xB0
    d = mxMath::Fabs( axisdir[2] * c[1][0] - axisdir[1] * c[2][0] );
    ...
    // axis C0 + t * A0xB1
    d = mxMath::Fabs( axisdir[2] * c[1][1] - axisdir[1] * c[2][1] );
    ...
    // axis C0 + t * A0xB2
    d = mxMath::Fabs( axisdir[2] * c[1][2] - axisdir[1] * c[2][2] );
    ...
    // axis C0 + t * A1xB0
    d = mxMath::Fabs( axisdir[0] * c[2][0] - axisdir[2] * c[0][0] );
    ...
    // axis C0 + t * A1xB1
    d = mxMath::Fabs( axisdir[0] * c[2][1] - axisdir[2] * c[0][1] );
    ...
    // axis C0 + t * A1xB2
    d = mxMath::Fabs( axisdir[0] * c[2][2] - axisdir[2] * c[0][2] );
    ...
    // axis C0 + t * A2xB0
    d = mxMath::Fabs( axisdir[1] * c[0][0] - axisdir[0] * c[1][0] );
    ...
    // axis C0 + t * A2xB1
    d = mxMath::Fabs( axisdir[1] * c[0][1] - axisdir[0] * c[1][1] );
    ...
    // axis C0 + t * A2xB2
    d = mxMath::Fabs( axisdir[1] * c[0][2] - axisdir[0] * c[1][2] );
...
bool OOBB::LineIntersection( const Vec3D &start, const Vec3D &end ) const {
    ...
    ld[0] = mxMath::Fabs( lineDir * axis[0] );
	if ( mxMath::Fabs( dir * axis[0] ) > extents[0] + ld[0] ) {
        return false;
	}

    ld[1] = mxMath::Fabs( lineDir * axis[1] );
	if ( mxMath::Fabs( dir * axis[1] ) > extents[1] + ld[1] ) {
        return false;
	}

    ld[2] = mxMath::Fabs( lineDir * axis[2] );
	if ( mxMath::Fabs( dir * axis[2] ) > extents[2] + ld[2] ) {
        return false;
	}

    Vec3D cross = lineDir.Cross( dir );

	if ( mxMath::Fabs( cross * axis[0] ) > extents[1] * ld[2] + extents[2] * ld[1] ) {
        return false;
	}

	if ( mxMath::Fabs( cross * axis[1] ) > extents[0] * ld[2] + extents[2] * ld[0] ) {
        return false;
	}

	if ( mxMath::Fabs( cross * axis[2] ) > extents[0] * ld[1] + extents[1] * ld[0] ) {
        return false;
	}
...
bool OOBB::RayIntersection( const Vec3D &start, const Vec3D &dir, FLOAT &scale1, FLOAT &scale2 ) const {
	...
	scale1 = -mxMath::INFINITY;
	scale2 = mxMath::INFINITY;
...
============
*/
void OOBB::FromPointRotation( const Vec3D &point, const mxRotation &rotation ) {
	// FIXME: implement
}
...
============
*/
void OOBB::FromBoxRotation( const OOBB &box, const mxRotation &rotation ) {
	// FIXME: implement
}
