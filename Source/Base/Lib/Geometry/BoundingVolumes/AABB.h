...
public:
	...
	const Vec3D &	operator[]( const mxUInt index ) const;
	Vec3D &			operator[]( const mxUInt index );
	...
	mxUInt			GetLargestAxis( void ) const;
	...
	void			FromBoundsRotation( const AABB &bounds, const Vec3D &origin, const Matrix3 &axis, const mxRotation &rotation );
	...
	const Vec3D *	GetVertexNormals() const;
	...
	const mxUInt *	GetEdges() const;

	//------------------------------------------------------------------------------------

	// Assuming you're standing in the center of a box;
	// the plane's normals point outwards.
	enum EBoxPlaneSides
	{
		AABB_SIDE_IN_FRONT,
		AABB_SIDE_BEHIND,
	...
	void	GetPlanes( mxPlane outPlanes[NUM_SIDES] ) const;
...
public:
	static const AABB	BOUNDS_ZERO;
	static const AABB	INFINITE_EXTENT;
};
...
FORCEINLINE const Vec3D &AABB::operator[]( const mxUInt index ) const {
	return mPoints[index];
}

FORCEINLINE Vec3D &AABB::operator[]( const mxUInt index ) {
	return mPoints[index];
}
...
FORCEINLINE AABB AABB::operator*( const Matrix3 &r ) const {
	AABB bounds;
	bounds.FromTransformedBounds( *this, Vec3D::vec3_origin, r );
	return bounds;
}

FORCEINLINE AABB &AABB::operator*=( const Matrix3 &r ) {
	this->FromTransformedBounds( *this, Vec3D::vec3_origin, r );
	return *this;
}
...
FORCEINLINE void AABB::Clear( void ) {
	mPoints[0][0] = mPoints[0][1] = mPoints[0][2] = mxMath::INFINITY;
	mPoints[1][0] = mPoints[1][1] = mPoints[1][2] = -mxMath::INFINITY;
}
...
FORCEINLINE mxUInt AABB::GetLargestAxis( void ) const
{
...
{	MX_OPTIMIZE( "Use SSE? Compare both methods:" )

#if 1 // correct method ( if the matrix is row-major, includes only translation and rotation )
	Vec3D center( GetCenter() );
	transform.TransformVector( center );

	Vec3D halfExtent( GetHalfSize() );

	Matrix4  mat;
	mat.SetIdentity();

	mat[0][0] = mxMath::Fabs( transform[0][0] );
	mat[0][1] = mxMath::Fabs( transform[0][1] );
	mat[0][2] = mxMath::Fabs( transform[0][2] );

	mat[1][0] = mxMath::Fabs( transform[1][0] );
	mat[1][1] = mxMath::Fabs( transform[1][1] );
	mat[1][2] = mxMath::Fabs( transform[1][2] );

	mat[2][0] = mxMath::Fabs( transform[2][0] );
	mat[2][1] = mxMath::Fabs( transform[2][1] );
	mat[2][2] = mxMath::Fabs( transform[2][2] );

	mat.TransformNormal( halfExtent );
...
FORCEINLINE AABB AABB::Rotate( const Matrix3 &rotation ) const {
	AABB bounds;
	bounds.FromTransformedBounds( *this, Vec3D::vec3_origin, rotation );
	return bounds;
}

FORCEINLINE AABB &AABB::RotateSelf( const Matrix3 &rotation ) {
	FromTransformedBounds( *this, Vec3D::vec3_origin, rotation );
	return *this;
}
...
FORCEINLINE void AABB::AxisProjection( const Vec3D &dir, FLOAT &min, FLOAT &max ) const {
	...
	d1 = dir * center;
	d2 = mxMath::Fabs( extents[0] * dir[0] ) +
			mxMath::Fabs( extents[1] * dir[1] ) +
				mxMath::Fabs( extents[2] * dir[2] );
...
FORCEINLINE void AABB::AxisProjection( const Vec3D &origin, const Matrix3 &axis, const Vec3D &dir, FLOAT &min, FLOAT &max ) const {
	...
	d1 = dir * center;
	d2 = mxMath::Fabs( extents[0] * ( dir * axis[0] ) ) +
			mxMath::Fabs( extents[1] * ( dir * axis[1] ) ) +
				mxMath::Fabs( extents[2] * ( dir * axis[2] ) );
