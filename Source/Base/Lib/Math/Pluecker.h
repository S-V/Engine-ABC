...
public:	
	...
	bool			FromPlanes( const mxPlane &p1, const mxPlane &p2 );			// pluecker from intersection of planes
...
FORCEINLINE bool mxPluecker::Compare( const mxPluecker &a, const FLOAT epsilon ) const {
	if ( mxMath::Fabs( p[0] - a[0] ) > epsilon ) {
		return false;
	}
			
	if ( mxMath::Fabs( p[1] - a[1] ) > epsilon ) {
		return false;
	}

	if ( mxMath::Fabs( p[2] - a[2] ) > epsilon ) {
		return false;
	}

	if ( mxMath::Fabs( p[3] - a[3] ) > epsilon ) {
		return false;
	}

	if ( mxMath::Fabs( p[4] - a[4] ) > epsilon ) {
		return false;
	}

	if ( mxMath::Fabs( p[5] - a[5] ) > epsilon ) {
		return false;
	}
...
FORCEINLINE FLOAT mxPluecker::Length( void ) const {
	return ( FLOAT )mxMath::Sqrt( p[5] * p[5] + p[4] * p[4] + p[2] * p[2] );
}
