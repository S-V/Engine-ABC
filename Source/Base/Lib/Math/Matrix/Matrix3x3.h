...
FORCEINLINE bool Matrix3::IsSymmetric( const FLOAT epsilon ) const {
	if ( mxMath::Fabs( mColumns[0][1] - mColumns[1][0] ) > epsilon ) {
		return false;
	}
	if ( mxMath::Fabs( mColumns[0][2] - mColumns[2][0] ) > epsilon ) {
		return false;
	}
	if ( mxMath::Fabs( mColumns[1][2] - mColumns[2][1] ) > epsilon ) {
		return false;
	}
	return true;
}
...
FORCEINLINE bool Matrix3::IsDiagonal( const FLOAT epsilon ) const {
	if ( mxMath::Fabs( mColumns[0][1] ) > epsilon ||
		mxMath::Fabs( mColumns[0][2] ) > epsilon ||
		mxMath::Fabs( mColumns[1][0] ) > epsilon ||
		mxMath::Fabs( mColumns[1][2] ) > epsilon ||
		mxMath::Fabs( mColumns[2][0] ) > epsilon ||
		mxMath::Fabs( mColumns[2][1] ) > epsilon ) {
		return false;
	}
	return true;
}
