...
============
*/
mxAngles Matrix3::ToAngles( void ) const {
	mxAngles	angles;
	DOUBLE		theta;
	DOUBLE		cp;
	FLOAT		sp;
	...
	if ( cp > 8192.0f * mxMath::FLT_EPSILON ) {
		angles.pitch	= RAD2DEG( theta );
		angles.yaw		= RAD2DEG( mxMath::ATan( mColumns[ 0 ][ 1 ], mColumns[ 0 ][ 0 ] ) );
		angles.roll		= RAD2DEG( mxMath::ATan( mColumns[ 1 ][ 2 ], mColumns[ 2 ][ 2 ] ) );
	} else {
		angles.pitch	= RAD2DEG( theta );
		angles.yaw		= RAD2DEG( -mxMath::ATan( mColumns[ 1 ][ 0 ], mColumns[ 1 ][ 1 ] ) );
		angles.roll		= 0;
	}
	return angles;
}
...
============
*/
mxCQuat Matrix3::ToCQuat( void ) const {
	Quat q = ToQuat();
	if ( q.w < 0.0f ) {
		return mxCQuat( -q.x, -q.y, -q.z );
	}
	return mxCQuat( q.x, q.y, q.z );
}
...
============
*/
mxRotation Matrix3::ToRotation( void ) const {
	mxRotation	r;
	FLOAT		trace;
	FLOAT		s;
	FLOAT		t;
	...
	r.angle = mxMath::ACos( r.angle );
	if ( mxMath::Fabs( r.angle ) < 1e-10f ) {
...
Vec3D Matrix3::ToAngularVelocity( void ) const {
	mxRotation rotation = ToRotation();
...
bool Matrix3::InverseFastSelf( void ) {
	...
	// r3.InverseSelf();
	if ( mxMath::Fabs( r3 ) < MATRIX_INVERSE_EPSILON ) {
		return false;
	}
