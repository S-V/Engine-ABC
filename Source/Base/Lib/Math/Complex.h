...
FORCEINLINE mxComplex mxComplex::operator/( const mxComplex &a ) const {
	FLOAT s, t;
	if ( mxMath::Fabs( a.r ) >= mxMath::Fabs( a.i ) ) {
		s = a.i / a.r;
		t = 1.0f / ( a.r + s * a.i );
...
FORCEINLINE mxComplex operator/( const FLOAT a, const mxComplex &b ) {
	FLOAT s, t;
	if ( mxMath::Fabs( b.r ) >= mxMath::Fabs( b.i ) ) {
		s = b.i / b.r;
		t = a / ( b.r + s * b.i );
...
FORCEINLINE mxComplex mxComplex::Reciprocal( void ) const {
	FLOAT s, t;
	if ( mxMath::Fabs( r ) >= mxMath::Fabs( i ) ) {
		s = i / r;
		t = 1.0f / ( r + s * i );
		return mxComplex( t, - s * t );
	} else {
...
FORCEINLINE mxComplex mxComplex::Sqrt( void ) const {
	...
	if ( x >= y ) {
		w = y / x;
		w = mxMath::Sqrt( x ) * mxMath::Sqrt( 0.5f * ( 1.0f + mxMath::Sqrt( 1.0f + w * w ) ) );
	} else {
		w = x / y;
		w = mxMath::Sqrt( y ) * mxMath::Sqrt( 0.5f * ( w + mxMath::Sqrt( 1.0f + w * w ) ) );
	}
	if ( w == 0.0f ) {
		return mxComplex( 0.0f, 0.0f );
	}
...
FORCEINLINE FLOAT mxComplex::Abs( void ) const {
	...
	if ( x == 0.0f ) {
		...
		t = y / x;
		return x * mxMath::Sqrt( 1.0f + t * t );
	} else {
		t = x / y;
		return y * mxMath::Sqrt( 1.0f + t * t );
	}
}
...
FORCEINLINE bool mxComplex::Compare( const mxComplex &a, const FLOAT epsilon ) const {
	if ( mxMath::Fabs( r - a.r ) > epsilon ) {
		return false;
	}
	if ( mxMath::Fabs( i - a.i ) > epsilon ) {
		return false;
	}
	return true;
}
